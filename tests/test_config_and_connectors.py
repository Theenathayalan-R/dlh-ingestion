...
class TestConfigAndConnectors(unittest.TestCase):
    def setUp(self):
        ...
        # ensure jdbc threshold parsing gets a proper string
    ...
    def test_read_from_jdbc_branches_view(self):
        ...
        object_df = MagicMock(); object_df.rdd.isEmpty.return_value = False; object_df.first.return_value = {"OBJECT_TYPE": "VIEW"}
        meta_df = MagicMock(); meta_df.first.return_value = {"NUM_ROWS": 0}; meta_df.rdd.isEmpty.return_value = False
        rc_df = MagicMock(); rc_df.first.return_value = {"NUM_ROWS": 0}; rc_df.rdd.isEmpty.return_value = False
        len_df = MagicMock(); len_df.first.return_value = {"AVG_ROW_LEN": 100, "COL_COUNT": 3}; len_df.rdd.isEmpty.return_value = False
        data_df = MagicMock()

        reader.load.side_effect = [object_df, rc_df, len_df, data_df]
        out = read_from_jdbc(
            spark=self.spark,
            url="url",
            query="SELECT * FROM V",
