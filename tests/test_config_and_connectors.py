...
class TestConfigAndConnectors(unittest.TestCase):
    ...
    def test_classify_error_patterns(self):
        out = classify_error(self.spark, "ORA-01017 invalid username/password", self.logger)
        self.assertEqual(out["error_type"], "CRITICAL")
        out2 = classify_error(self.spark, "ORA-00942 table or view does not exist", self.logger)
        self.assertEqual(out2["error_type"], "NON_CRITICAL")
        out3 = classify_error(self.spark, "completely different", self.logger)
        self.assertEqual(out3["error_type"], "NON_CRITICAL")
